...
error = map(IRL_val - IRR_val, -600, 600, -100, 100);
integral += error;
integral = constrain(integral, -1000, 1000);
float derivative = error - lastError;
float correction = (Kp * error) + (Ki * integral) + (Kd * derivative);
    
int dynamicBase = baseSpeed - abs(error) * 0.05;
dynamicBase = constrain(dynamicBase, 80, baseSpeed);
